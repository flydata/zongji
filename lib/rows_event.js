...
RowsEvent.prototype._fetchOneRow = function(parser) {
  ...
  console.log(`Fetching one row, tablemap: ${JSON.stringify(tablemap)}. this.columns_present_bitmap: ${this.columns_present_bitmap}, this.tableId: ${this.tableId}`);
...
const readRow = function(parser, tablemap, bitmap, zongji) {
  ...
  console.log(`readRow, nullBitmapSize: ${nullBitmapSize}`);
  const nullBitmap = parser.parseBuffer(nullBitmapSize);
  console.log(`readRow, nullBitmap: ${nullBitmap}`);
  ...
  for (let i = 0, nullBitIndex = 0; i < tablemap.columns.length; i++) {
    ...
    if (getBit(nullBitmap, nullBitIndex) != 0) {
      ...
      console.log(`Column: ${column.name}, type: ${columnSchema.COLUMN_TYPE}`);
