...
const parseUInt64 = exports.parseUInt64 = function(parser) {
  ...
  return bigInt(TWO_TO_POWER_THIRTY_TWO).multiply(high).add(low).value;
};
...
const parseAnyInt = function(parser, column, columnSchema) {
  ...
  if (columnSchema.COLUMN_TYPE.indexOf('unsigned') === -1) {
    ...
    if (!int64 && (result & (1 << (length - 1)))) {
      ...
      result = bigInt(result).xor(Max64BitNumber).add(1).multiply(-1).value;
    }
  }
  return result;
};
...
// Rounds the time if it exceeds 24 hours (Issue on Redshift). Also returns a full
// date time string (No issue when writing to Redshift Time).
//
// Input: 28:00:00
// Output: 0001-01-02 04:00:00
const roundTime = function(value) {
  ...
  if (timeSplit && timeSplit.length == 3) {
    ...
    let time = moment("0001-01-01 00:00:00");
    time.add(hour, "hour");
    time.add(minute, "minute");
    time.add(second, "second");
...
const parseGeometryValue = function(buffer) {
  ...
  return buffer.toString('hex').slice(8);
  ...
  )
{
  ...
  switch (column.type) {
    ...
    case MysqlTypes.JSON:
      ...
      result = buffer.length === 0 ? null : JSON.parse(decodeJson(buffer));
      break;
    ...
    case MysqlTypes.TIME2:
      ...
      } else if (zongji.roundTime) {
        result = roundTime(result);
      }
      break;
    case MysqlTypes.DATETIME:
      raw = Number(parseUInt64(parser));
    ...
    case MysqlTypes.TIMESTAMP:
      raw = parser.parseUnsignedNumber(4);
      result = dtDecode.getTimeStamp(zongji.connection.config.dateStrings, zongji.timezone, raw);
      break;
